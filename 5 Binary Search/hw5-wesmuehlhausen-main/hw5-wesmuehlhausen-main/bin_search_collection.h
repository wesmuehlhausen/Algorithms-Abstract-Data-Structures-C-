...
public:
    ...
    void keys(ArrayList<K>& all_keys) const;//returns list if keys
    void sort(ArrayList<K>& all_keys_sorted) const;//defers to keys
...
void BinSearchCollection<K, V>::remove(const K& a_key)
{
	...
	//{
		size_t index = 0;
		if(bin_search(a_key, index))//set correct index
