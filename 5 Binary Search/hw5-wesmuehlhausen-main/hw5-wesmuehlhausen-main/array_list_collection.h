...
// DATE: 10 / 12 / 2020
// DESC: implements add, remove, find, find range, and sort functions which rely
...
public://public functions
    ...
    void remove(const K& a_key);//remove key
    bool find(const K& search_key, V& the_val) const;//find key
    void find(const K& k1, const K& k2, ArrayList<K>& keys) const;//find range
    void keys(ArrayList<K>& all_keys) const;//gathers all keys
    void sort(ArrayList<K>& all_keys_sorted) const;//sorts using quicksort
